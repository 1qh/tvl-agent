...
import type { CarouselApi } from '@a/ui/carousel'
import type { ComponentProps } from 'react'

import { cn } from '@a/ui'
import { Badge } from '@a/ui/badge'
import { Carousel, CarouselContent, CarouselItem } from '@a/ui/carousel'
...
export const InlineCitationCarouselIndex = ({ children, className, ...props }: InlineCitationCarouselIndexProps) => {
  ...
  return (
    <div
      className={cn('flex flex-1 items-center justify-end px-3 py-1 text-xs text-muted-foreground', className)}
      {...props}>
...
export const InlineCitationSource = ({
  ...
  <div className={cn('space-y-1', className)} {...props}>
    {title ? <h4 className='truncate text-sm leading-tight font-medium'>{title}</h4> : null}
    {url ? <p className='truncate text-xs break-all text-muted-foreground'>{url}</p> : null}
...
export const InlineCitationQuote = ({ children, className, ...props }: InlineCitationQuoteProps) => (
  <blockquote className={cn('border-l-2 border-muted pl-3 text-sm text-muted-foreground italic', className)} {...props}>
    {children}
  </blockquote>
)
