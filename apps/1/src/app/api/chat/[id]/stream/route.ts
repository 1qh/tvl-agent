...
import { getChatById, getMessagesByChatId, getStreamIdsByChatId } from '~/lib/db'
import ChatSDKError from '~/lib/errors'

import { getStreamContext } from '../../route'

const GET = async (_: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  if (!stream) {
    const messages = await getMessagesByChatId({ id: chatId }),
      mostRecentMessage = messages.at(-1)
    ...
    const restoredStream = createUIMessageStream<ChatMessage>({
      execute: ({ writer }) => {
        writer.write({ data: JSON.stringify(mostRecentMessage), transient: true, type: 'data-appendMessage' })
      }
    })
